...
router.get('/accounts/allaccounts/:value',async (req,res)=>{
    try {
        ...
        const result = await sql.query(`SELECT * FROM WEBID ORDER BY '${orderBy}'`)
...
router.post('/accounts/searchaccounts', async (req,res)=>{
    try {
      ...
      const result = await sql.query(`SELECT * FROM WEBID WHERE ${searchBy} LIKE '%${searchedValue}%' ORDER BY '${searchBy}'`)
      res.status(200).json(result.recordset)
    } catch (error) {
      res.status(500).json({message:"Internal server error: "+error})
      console.log(error)
